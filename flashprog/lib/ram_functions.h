...
uint8_t readRAM(uint16_t addr) {
	...
	_delay_us(1);					// pause
	setPin(&PORTD, R_CE, LOW); 		// Enable /R_CE
	setPin(&PORTD, R_OE, LOW); 		// Enable /R_OE
	_delay_us(10);					// pause
	data = dataport.readInput();	// Read data port
	setPin(&PORTD, R_OE, HIGH);		// Disable /R_OE
	setPin(&PORTD, R_CE, HIGH);		// Disable /R_CE
	return data;
}
...
void writeRAM(uint16_t addr, uint8_t data) {
	...
	// _delay_us(1);					// pause
	setPin(&PORTD, R_CE, LOW); 		// Enable /R_CE
	setPin(&PORTD, R_WE, LOW); 		// Enable /R_WE
	_delay_us(10);					// pause
	setPin(&PORTD, R_WE, HIGH);		// Disable /R_WE
	setPin(&PORTD, R_CE, HIGH);		// Disable /R_CE
}
