...
namespace smd_avr_i2c {
	...
	void I2C_enable_pullups(volatile uint8_t* i2cPort,
		uint8_t sdaGpio, uint8_t sclGpio) {
	...
	uint8_t I2C_start(uint8_t address, uint8_t mode) {
		uint8_t status = 0;
		// Sets the start condition, then loads the peripheral address with the
		// read or write bit and sends that.
		// Write start condition: set (ie, clear) interrupt, set enable and set
		// start condition
		TWCONTROL_REG = ((1 << TWINT) | (1 << TWEN) | (1 << TWSTA));
		I2C_wait_for_completion;			// blocking!!
		if ((TWSTAT_REG & I2C_STATUS_MASK) == I2C_TX_STARTED ||
			(TWSTAT_REG & I2C_STATUS_MASK) == I2C_RX_RESTARTED) {

			// (TWSTAT_REG & I2C_STATUS_MASK) at this point should be 0x08 -
			// start condition transmitted.
			// Put address of peripheral into data register, plus read or
			// write bit
...
/**
	...
	@param address			Hex address of peripheral device.
**/
...
/**
	Constructor using selectable I2C bus speeds of 100kHz or 400kHz and
...
/**
	...
	@param prescaler_mode	Values 0-3. Default is 0.
**/
void SMD_I2C_Device::_init(uint8_t address, unsigned long busSpeed,
	uint8_t prescaler_mode) {
...
uint8_t SMD_I2C_Device::readRegister(uint8_t regAddr) {
	...
	_error = start(I2C_WRITE_MODE);
	if (_error == 0) {
		sendByte(regAddr);
		// _delay_us(10);
		_error = start(I2C_READ_MODE);
		if (_error == 0) {
			regVal = readNoAck();
		} else {
			readNoAck();
...
// Wrapper to readWord(uint8_t byteOrder) below
...
uint8_t SMD_I2C_Device::sendWord(uint16_t wordData) {
	// use current byte order setting
	uint8_t error = sendWord(wordData, _byteOrder);
	return error;
}
...
void SMD_I2C_Device::setByteOrder(uint8_t byteOrder) {
	if (byteOrder == I2C_BYTE_ORDER_LSB_FIRST
		|| byteOrder == I2C_BYTE_ORDER_MSB_FIRST) {
		_byteOrder = byteOrder;
	}
}

void SMD_I2C_Device::setPause(uint8_t delay) {
	_postFuncPause = delay;
}

uint8_t SMD_I2C_Device::start(void) {
	// Start without setting mode or sending address
...
uint8_t SMD_I2C_Device::start(uint8_t mode) {
	...
	// -  I2C_WRITE_MODE  = 0
	// -  I2C_READ_MODE   = 1
	// This version sets the start condition, then loads the peripheral address
	// with the read or write bit and sends that.
	uint8_t err = 0;
	// Write start condition: set (ie, clear) interrupt, set enable and set start condition
	TWCONTROL_REG = ((1 << TWINT) | (1 << TWEN) | (1 << TWSTA));
	_waitForComplete();
	if ((TWSTAT_REG & I2C_STATUS_MASK) == I2C_TX_STARTED ||
		(TWSTAT_REG & I2C_STATUS_MASK) == I2C_TX_RESTARTED ||
		(TWSTAT_REG & I2C_STATUS_MASK) == I2C_RX_RESTARTED) {
		// (TWSTAT_REG & I2C_STATUS_MASK) at this point should be 0x08 (start
		// condition transmitted) - or 0x10 (restart transmitted)
		// Put address of peripheral into data register, plus read or write bit.
...
void SMD_I2C_Device::_pause() {

	if (_postFuncPause > 0) {
		uint8_t delay = _postFuncPause;
		// Have to do this weird loop because the _delay_us() function demands
